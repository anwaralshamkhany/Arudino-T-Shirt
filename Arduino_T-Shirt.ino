...
void setup(void)
{
  ...
  Serial.println(F("Adding the muscles flexion (UUID = 0x1809): "));
  htsServiceId = gatt.addService(0x1809);
  if (htsServiceId == 0) {
    error(F("Could not locate muscle adjust noedes"));
  }
  
  /* Add the Temperature Measurement characteristic which is composed of
   * 1 byte flags + 4 float */
  /* Chars ID for Measurement should be 1 */
  Serial.println(F("muscle flex (UUID = 0x2A1C): "));
  ...
  if (htsMeasureCharId == 0) {
    error(F("Could not add muscle characteristic"));
  }

  Serial.print(F("Adding muslce Service UUID to the advertising payload: "));
...
void loop(void)
{
  ...
  Serial.print(F("Updating muscle value to "));
  Serial.print(temp);
